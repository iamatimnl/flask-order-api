...
def mollie_webhook():
    ...
    if info.get('status') == 'paid':
        ...
        # 🔥 改为向 POS 请求订单详情
        try:
            pos_response = requests.get(f"https://nova-asia.onrender.com/api/orders/{order_id}", timeout=5)
            if pos_response.status_code != 200:
                print(f"❌ POS API fout: {pos_response.status_code}")
                return '', 400
            order = pos_response.json()
        except Exception as e:
            print(f"❌ POS API netwerkfout: {e}")
            return '', 400
